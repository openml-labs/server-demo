...
class Base(DeclarativeBase, MappedAsDataclass):
    ...
    def to_dict(self, depth: int = 1) -> dict:
        """ Serializes all fields of the dataclasses, as well as its references, up to a certain depth.

        Whenever the attributes are themselves dataclasses, such as Datasets referencing Publications,
        these dataclasses may refer back to other dataclasses, possibly in a cyclic manner.
        For this reason, using dataclasses.to_dict(object) results in infinite recursion.
        To prevent this from happening, we define this method which only recurs a `depth` amount of time.
        ...
        depth, int (default=1): dictates how many levels of object references to jsonify.
          ...
          E.g., for depth=1 a Dataset will include Publications in its JSON, but not the Publictions' Datasets.
        """
        d = {}
        for field in dataclasses.fields(self):
            ...
            if isinstance(value, Base):
                ...
                if all(isinstance(item, Base) for item in value):
                    ...
                    raise NotImplemented("Serializing mixed-type lists is not supported.")
                else:
                    d[field.name] = value
            else:
                d[field.name] = value
