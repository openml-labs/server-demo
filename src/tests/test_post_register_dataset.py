...
def test_missing_value(client: TestClient, engine: Engine):
    response = client.post("/register/dataset", json={"platform": "openml", "platform_identifier": "1"})  # name omitted
...
def test_null_value(client: TestClient, engine: Engine):
    response = client.post("/register/dataset", json={"name": None, "platform": "openml", "platform_identifier": "1"})
    assert response.status_code == 422
    assert response.json()['detail'] == [{'loc': ['body', 'name'],
                                          'msg': 'field required',
                                          'type': 'value_error.missing'}]
